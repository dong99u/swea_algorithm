...
def get_synergy(food_list, synergy):
    """
...
for test_case in range(1, T+1):
    ...
    comb_list = combinations(num_list, N//2)

    # 최댓값 설정
    res = sum(sum(data, []))
    # 식재료 조합을 순회하며
    # A 요리와 B 요리의 시너지 합을 구한다.
    for a_food_list in comb_list:
        ...
        # 함수화 진행
        a_synergy_sum = get_synergy(a_food_list, data)
        b_synergy_sum = get_synergy(b_food_list, data)
